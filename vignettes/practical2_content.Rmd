...
```
7. Create a `summary` method for the `cohort` class. When the `summary` function is called on a `cohort` object it should call the base `summary` on the `details`  element.
8. Use the `body` function to check if the function is already a generic function.
9. Use the `args` function to determine the arguments.
10. Create a `summary.cohort` function
```{r }
...
```
11. Create a `hist` method for the `cohort` class. When the `hist` function is called on a `cohort` object, it should produce a single plot showing two histograms - one for height and another for weight.
```{r }
