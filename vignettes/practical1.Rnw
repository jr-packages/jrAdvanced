...
\begin{enumerate}
\item Create an \cc{.Rprofile} file. An easy way of creating the file is to use 
the R function \cc{file.create}, so
<<eval=FALSE>>=
...
\item Try adding a few functions to your \cc{.Rprofile}.\sidenote{See chapter 2 in the notes.}
Use the  hidden environment trick. Also take a look at this stackoverflow question 
\begin{center}
...
% 
% A common argument\sidenote{Especially when dealing with S3 objects and functions.} is \texttt{...}. We can explore what happens using the \cc{print\_dots} function in the \cc{jrAdvanced} package
% 
% <<tidy=FALSE>>=
% library("jrAdvanced")
